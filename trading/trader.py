...
class Trader:
    ...
    async def __trading(
        ...
        async for market_data in self.__stream_service.start_async_market_data_stream(
    ...
    def __process_new_candles(
        ...
        for strategy in strategies[candle.figi]:
            ...
            if current_trade_order:
                ...
                if low <= current_trade_order.signal.stop_loss_level <= high:
                    ...
                    self.__blogger.send_text_message(f"STOP LOSS: {candle.figi}")
                    # close_order_id = \
                    ...
                    self.__blogger.send_text_message(f"TAKE PROFIT: {candle.figi}")
                    # close_order_id = \
                    #     self.__close_position_by_figi(account_id, [candle.figi], strategies).get(candle.figi, None)
                    # if close_order_id:
                    #     trade_order = self.__today_trade_results.close_position(candle.figi, close_order_id)
            ...
            if candle.time > current_figi_candle.time and \
                ...
                signal_new = strategy.analyze_candles(
                ...
                if signal_new:
                    ...
                    if self.__today_trade_results.get_current_trade_order(candle.figi):
                        ...
                        if available_lots:
                            ...
                            self.__blogger.send_text_message(f"Open position: {candle.figi}")
                        else:
    ...
    def __get_today_strategies(self, strategies: list[IStrategy]) -> dict[str, list[IStrategy]]:
        ...
        for strategy in strategies:
            ...
            if (not share_settings.otc_flag) \
                ...
                if strategy.settings.figi not in today_trade_strategy.keys():
                    today_trade_strategy[strategy.settings.figi] = []
